...
`Save` を押すと、プロンプト、Temperature、現在のプリセット識別情報が `engine/prompts.json` と、現在有効なプリセットに対応する `Preset` フォルダ内のCSVへ保存され、次の会話ターンから反映されます。設定を保存するまでは組み込みの標準プロンプトが使用されます。
...
`New` はプリセット名が未入力でも使用でき、その場合は `New preset`、`New preset 2` のような重複しない名前が自動的に付けられます。`Save As` と名称変更ではプリセット名の入力が必要です。
...
前回使用したプリセットのCSVが起動前に削除されていた場合は、`Default` プリセットを優先して自動選択します。`Default` も存在しない場合は利用可能な先頭のプリセットへ切り替わり、CSVが1件もない場合は組み込みの標準設定へ戻ります。
